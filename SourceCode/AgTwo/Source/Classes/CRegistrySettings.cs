...
namespace AgTwo
{
    public static class RegistrySettings
    {
        ...
        public static string profileDirectory =
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "AgOpenGPS", "AgTwo");
        ...
        public static void Load()
        {
            try
            {
                ...
                //keep below 500 kb
                Log.CheckLogSize(Path.Combine(logsDirectory, "AgTwo_Events_Log.txt"), 500000);
                ...
                if (regKey == null)
                {
                    ...
                    Key.SetValue("ProfileNameTwo", "Default Profile");
                    Key.Close();
                    Log.EventWriter("Registry -> SubKey AgTwo and Keys Created\r");
                }
                else
                {
                    try
                    {
                        ...
                        if (regKey.GetValue("ProfileNameTwo") == null || regKey.GetValue("ProfileNameTwo").ToString() == "")
                        {
                            RegistryKey key = Registry.CurrentUser.CreateSubKey(@"SOFTWARE\AgOpenGPS");
                            key.SetValue("ProfileNameTwo", "Default Profile");
                        ...
                        {
                            profileName = regKey.GetValue("ProfileNameTwo").ToString();
        ...
        public static void Save()
        {
            ...
            try
            {
                key.SetValue("ProfileNameTwo", profileName);
